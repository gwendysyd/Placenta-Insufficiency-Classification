...
We will apply different neural network architectures to the datasets in their original form to assess the effectiveness of each model and to get a baseline. We introduce image augmentation, with a more aggressive augmentation approach, and test the approach with and without normalized class weights.
