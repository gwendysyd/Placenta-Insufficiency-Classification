...
We will apply different neural network architectures to the datasets in their original form to assess the effectiveness of each model and to get a baseline. We apply image augmentation, and test the approach with and without normalized class weights. In this case, we use the images resulting after applying the original mask expanded by a ratio of 1.3x.
